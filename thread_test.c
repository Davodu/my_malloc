...
#define MALLOC(sz) ts_malloc(sz)
#define FREE(p)    ts_free(p)
...
void do_allocate(int thread_id) {
    ...
    //For MacOS, this is commented out since barriers are not supported.
    //Please note you may want to increase the NUM_ITEMS to a larger
    //number when testing your thread safe implementation without the
    //barrier to ensure that threads are calling malloc() and free()
    //concurrently.
